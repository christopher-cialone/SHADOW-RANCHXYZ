...
export function Header({ onThemeToggle, currentTheme }: HeaderProps) {
  ...
  return (
    <>
      ...
      <header 
        className="fixed top-0 left-0 right-0 z-50 bg-tech-purple-900/95 backdrop-blur-sm border-b border-tech-cyan-600/30"
      ...
      >
        <div className="container mx-auto px-3 py-2 sm:px-4 sm:py-3 flex items-center justify-between h-full">
          ...
          >
            <svg
              className="w-6 h-6 text-tech-cyan-400"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <Link href="/">
            <div className="flex items-center space-x-2 sm:space-x-4 cursor-pointer touch-target">
              <span className="text-tech-cyan-400 text-lg sm:text-2xl animate-tech-pulse">🏜️</span>
              <h1 className="font-data70 bg-gradient-to-r from-tech-cyan-400 to-tech-purple-400 bg-clip-text text-transparent text-lg sm:text-xl md:text-2xl lg:text-[30px] leading-tight">
                brb: shadow ranch
              </h1>
            </div>
          </Link>
          ...
          <nav className="hidden md:flex items-center space-x-6">
            {navItems.map((item) => (
              <Link key={item.href} href={item.href}>
                <span
                  className={cn(
                    "font-mono text-sm lg:text-base text-gray-300 hover:text-tech-cyan-400 transition-colors uppercase tracking-wider cursor-pointer touch-target flex items-center justify-center px-3 py-2 rounded-lg",
                    location === item.href && "text-tech-cyan-400 bg-tech-cyan-400/10"
                  )}
                >
                  {item.label}
                </span>
